	...
	var a = k[m.k] = {
		...
		f : function() {
			return {
				...
				get : function(b, c) {
					var d = null;
					return d = typeof b[c] === "string" ? b[c] : b
							.getAttribute(c)
				},
				set : function(b, c, d) {
				...
				getSelection : function() {
					return ("" + (a.w.getSelection ? a.w.getSelection()
							: a.d.getSelection ? a.d.getSelection()
									: a.d.selection.createRange().text))
							.replace(/(^\s+|\s+$)/g, "")
				},
				log : function(b, c) {
					b = "?page=" + encodeURIComponent(a.d.URL) + "&reason="
							+ encodeURIComponent(b);
					if (c)
						b = b + "&image=" + encodeURIComponent(c);
					a.f.call(a.a.log + b, a.f.ping.log)
				},
				pin : function(b) {
					var c = a.a.pin + "?", d = "false", e = a.f
							.get(b, "pinImg"), g = a.f.get(b, "pinUrl")
							|| a.d.URL, f = a.v.selectedText
							|| a.f.get(b, "pinDesc") || a.d.title;
					g = g.split("#")[0];
					if (b.rel === "video")
						d = "true";
					...
					c = c + "&title="
							+ encodeURIComponent(a.d.title.substr(0, 500));
					c = c + "&is_video=" + d;
					c = c + "&description="
							+ encodeURIComponent(f.substr(0, 500));
					if (a.v.inlineHandler)
						c = c + "&via=" + encodeURIComponent(a.d.URL);
					if (a.v.hazIOS)
						a.w.location = "http://" + c;
					else
						a.w.open("http://" + c, "pin" + (new Date).getTime(),
								a.a.pop)
				},
				close : function(b) {
					if (a.s.bg) {
				...
				click : function(b) {
					b = b || a.w.event;
					var c = null;
					if (c = b.target ? b.target.nodeType === 3 ? b.target.parentNode
							: b.target
							: b.srcElement)
						if (c === a.s.x)
							a.f.close();
						else if (c.className !== a.a.k + "_hideMe")
							if (c.className === a.a.k + "_pinThis") {
								a.f.pin(c);
				...
				presentation : function() {
					...
					}), c = a.a.cdn[a.w.location.protocol] || a.a.cdn["http:"], d = a.a.rules
							.join("\n");
				...
				thumb : function(b) {
					if (b.src) {
						if (!b.media)
							b.media = "image";
						var c = a.a.k + "_thumb_" + b.src, d = a.f.make({
							SPAN : {
						...
						if (a.v.canonicalTitle || b.title)
							a.f
									.set(e, "pinDesc", a.v.canonicalTitle
											|| b.title);
						...
						f.onload = function() {
							...
							if (h > i) {
								...
								this.style.marginTop = 0
										- (this.height - a.a.thumbCellSize) / 2
										+ "px"
							}
							if (h < i) {
								...
								this.style.marginLeft = 0
										- (this.width - a.a.thumbCellSize) / 2
										+ "px"
							}
						...
						if (e !== false)
							if (e = a.d.getElementById(e)) {
								...
								b.page || b.media !== "image" ? a.f.addThumb(
										a.s.embedContainer, d, "SPAN") : a.f
										.addThumb(a.s.imgContainer, d, "SPAN");
						else {
							a.s.imgContainer.appendChild(d);
							a.v.hazAtLeastOneGoodThumb += 1
						}
						(b = a.d.getElementById(c))
				...
				call : function(b, c) {
					var d = a.f.callback.length, e = a.a.k + ".f.callback[" + d
				...
				ping : {
					...
					checkDomain : function(b) {
						...
						if (b && b.disallowed_domains
							...
							c = 0;
							for (d = a.v.thumbed.length; c < d; c += 1)
								if (a.v.badDomain[a.f.get(a.v.thumbed[c],
										"domain")] === true) {
									b = a.v.thumbed[c].id.split("thumb_")[1];
									a.f.log("image_from_blacklisted_domain", b);
									a.f.unThumb(a.v.thumbed[c].id.split(
											"thumb_").pop())
								}
						}
					},
					info : function(b) {
						if (b)
							if (b.err) {
								a.f.log("api_nopin");
								a.f.unThumb(b.id)
							} else if (b.reply && b.reply.img
									&& b.reply.img.src) {
								var c = "";
								if (b.reply.page)
									c = b.reply.page;
								if (b.reply.nopin && b.reply.nopin === 1) {
									a.f.log("api_nopin", b.reply.img.src);
									a.f.unThumb(b.id)
								} else {
				...
				unThumb : function(b) {
					...
					if (c) {
						if (a.v.canonicalImage)
							if (a.a.k + "_thumb_" + a.v.canonicalImage === b)
								return;
				...
				seekCanonical : function(b) {
					var c = a.a.seek[b], d = null, e = null, g, f, i, h, l, j = {
						pPrice : "",
						pCurrencySymbol : ""
					};
					if (!c || !c.via)
					...
					if (j.pUrl && j.pImg) {
						...
						if (c.fixTitle)
							if (a.v.canonicalTitle.match(c.fixTitle.find)) {
								a.v.canonicalTitle = a.v.canonicalTitle
										.replace(c.fixTitle.find,
												c.fixTitle.replace);
								if (c.fixTitle.suffix)
									a.v.canonicalTitle += c.fixTitle.suffix
							}
						a.v.canonicalTitle = a.v.canonicalTitle.replace(/%s%/,
								j.pCurrencySymbol + j.pPrice);
						e.title = a.v.canonicalTitle;
						e.setAttribute("page", j.pUrl);
						if (c.fixImg)
							if (j.pImg.match(c.fixImg.find))
								j.pImg = j.pImg.replace(c.fixImg.find,
										c.fixImg.replace);
						if (c.checkNonCanonicalImages)
							a.v.checkNonCanonicalImages = true;
				...
				hazUrl : {
					...
					fivehundredpx : function() {
						...
						if (b) {
							...
							a.f.getExtendedInfo("src=fivehundredpx&id="
					...
					flickr : function() {
						...
						if (b) {
							b.setAttribute("extended", true);
							b.setAttribute("dupe", b.src);
							a.f.getExtendedInfo("src=flickr&id="
									+ encodeURIComponent(b.src))
						}
					},
					...
					soundcloud : function() {
						...
						if (b) {
							...
							a.f.getExtendedInfo("src=soundcloud&id="
									+ encodeURIComponent(a.d.URL.split("?")[0]
											.split("#")[0]))
						}
					},
					slideshare : function() {
						...
						if (b) {
							b.setAttribute("extended", true);
							a.v.scragAllThumbs = true;
							a.f.getExtendedInfo("src=slideshare&id="
									+ encodeURIComponent(a.d.URL.split("?")[0]
											.split("#")[0]))
						}
					},
					youtube : function() {
						...
						if (b) {
							b.setAttribute("extended", true);
							a.f.getExtendedInfo("src=youtube&id="
									+ encodeURIComponent(b.getAttribute("page")
											.split("=")[1].split("&")[0]))
						}
					},
					vimeo : function() {
						...
						if (a.w.location.protocol === "https:")
							c += "_s";
					...
					facebook : function() {
						a.f.close(a.a.msg.privateDomain.replace(
								/%privateDomain%/, "Facebook"))
					},
					googleReader : function() {
						a.f.close(a.a.msg.privateDomain.replace(
								/%privateDomain%/, "Google Reader"))
					},
				...
				hazSite : {
					behance : {
						img : function(b) {
							if (b.src) {
								b.src = b.src.split("?")[0];
								a.f.getExtendedInfo("src=behance&id="
										+ encodeURIComponent(b.src))
							}
						}
					},
					dasauge : {
						img : function(b) {
							b.src
									&& a.f.getExtendedInfo("src=dasauge&id="
											+ encodeURIComponent(b.src))
						}
					},
					dreamstime : {
						img : function(b) {
							if (b.src) {
								b.src = b.src.split("?")[0];
								a.f.getExtendedInfo("src=dreamstime&id="
										+ encodeURIComponent(b.src))
							}
						}
					},
						...
						img : function(b) {
							if (b = b.src.split("?")[0].split("#")[0]
								...
								id > 1E3
										...
										&& a.v.inlineHandler === "netflix"
										&& a.f
												.getExtendedInfo("src=netflix&id="
														+ id)
							}
						}
					},
					polyvore : {
						img : function(b) {
							b.src
									&& a.f.getExtendedInfo("src=polyvore&id="
											+ encodeURIComponent(b.src))
						}
					},
					youtube : {
						...
						video : function(b) {
							(b = a.f.get(b, "data-youtube-id"))
									&& a.f.getExtendedInfo("src=youtube&id="
											+ b)
						},
						embed : function(b) {
							var c = a.f.get(b, "flashvars"), d = "";
							if (c) {
								if (d = c.split("video_id=")[1])
									d = d.split("&")[0];
							...
							if (b)
								(c = a.f.getId({
									u : b
										...
										&& a.f
												.getExtendedInfo("src=youtube&id="
														+ c)
						}
					},
					vimeo : {
				...
				handleInline : {
					...
					tumblr : function(b) {
						...
						c = "";
						if (b.src) {
							for (c = b.parentNode; c.tagName !== "LI"
									&& c !== a.d.b;)
								c = c.parentNode;
							if (c.tagName === "LI"
									&& c.parentNode.id === "posts") {
				...
				hazTag : {
					img : function(b) {
						if (a.v.inlineHandler
								&& typeof a.f.handleInline[a.v.inlineHandler] === "function")
							a.f.handleInline[a.v.inlineHandler](b);
						else if (!b.src.match(/^data/)) {
							if (b.height > a.a.minImgSize
									&& b.width > a.a.minImgSize) {
								if (b.parentNode.tagName === "A"
										&& b.parentNode.href) {
									var c = b.parentNode, d = c.href.split(".")
											.pop().split("?")[0].split("#")[0];
									if (d === "gif" || d === "jpg"
											|| d === "jpeg" || d === "png") {
										d = new Image;
				...
				checkTags : function() {
					...
					for (c = a.a.check.length; b < c; b += 1) {
						...
						for (e = g.length; d < e; d += 1) {
							f = g[d];
							a.f.get(f, "nopin")
									&& !a.f.get(f, "pinterestCreated")
									&& a.f
											.log("image_with_inline_nopin",
													f.src);
							!a.f.get(f, "nopin") && f.style.display !== "none"
									&& f.style.visibility !== "hidden"
									&& a.v.tag.push(f)
						}
					}
				...
				getHeight : function() {
					return Math.max(Math.max(a.d.b.scrollHeight,
							a.d.d.scrollHeight), Math.max(a.d.b.offsetHeight,
							a.d.d.offsetHeight), Math.max(a.d.b.clientHeight,
							a.d.d.clientHeight))
				},
				structure : function() {
					a.s.shim = a.f.make({
				...
				checkDomainBlacklist : function() {
					...
					for (c in a.v.checkThisDomain)
						if (a.v.checkThisDomain[c].hasOwnProperty
				...
				getArgs : function() {
					var b = a.d.getElementsByTagName("SCRIPT"), c = b.length, d = 0;
					for (d = 0; d < c; d += 1)
						if (b[d].src.match(a.a.me)) {
							if (b[d].getAttribute("noCancel"))
								a.a.noCancel = true;
							if (b[d].getAttribute("noHeader"))
								a.a.noHeader = true;
							a.w.setTimeout(function() {
								...
								if (a.f.checkPage())
									if (a.v.hazGoodUrl === true) {
										...
										if (a.f.callback.length > 1)
											a.v.waitForCallbacks = a.w
													.setInterval(
															function() {
																if (a.v.awaitingCallbacks === 0)
																	if (a.v.hazAtLeastOneGoodThumb === 0
																			|| a.v.tag.length === 0) {
																		a.w
																				.clearInterval(a.v.waitForCallbacks);
																		a.f
																				.close(a.a.msg.notFound)
																	}
															}, 500);
										else if (!a.v.canonicalImage
												&& (a.v.hazAtLeastOneGoodThumb === 0 || a.v.tag.length === 0))
	...
	a.f.init()
		...
		(
				...
				navigator,
				{
					...
					maxCheckCount : 20,
					thumbCellSize : 200,
					check : [ "meta", "iframe", "embed", "object", "img",
							"video", "a" ],
					url : {
					...
					tag : {
						img : {
							dasauge : {
								att : "src",
								match : [ /^https?:\/\/cdn?[0-9]\.dasauge\.net/ ]
							},
							dreamstime : {
								att : "src",
							...
							netflix : {
								att : "src",
								match : [ /^http:\/\/cdn-?[0-9]\.nflximg\.com/,
										/^http?s:\/\/netflix\.hs\.llnwd\.net/ ]
							},
							polyvore : {
								att : "src",
								match : [ /^https?:\/\/(.*?)\.polyvoreimg\.com/ ]
							},
							youtube : {
								att : "src",
								match : [ /ytimg.com\/vi/,
					...
					wpop : "status=no,resizable=yes,scrollbars=yes,personalbar=no,directories=no,location=no,toolbar=no,menubar=no,width=632,height=270,left=0,top=0",
					rules : [
							...
							"#_bd #_hd #_logo {height: 43px; width: 100px; display: inline-block; margin-right: -100px; background: transparent url(_cdn/images/LogoRed.png) 50% 50% no-repeat; border: none;}",
							"@media only screen and (-webkit-min-device-pixel-ratio: 2) { #_bd #_hd #_logo {background-size: 100px 26px; background-image: url(_cdn/images/LogoRed.2x.png); } }",
							...
							"#_bd span._pinContainer cite span._dasauge { background: transparent url(_cdn/images/attrib/dasauge.png) 180px 3px no-repeat; }",
							...
							"#_bd span._pinContainer cite span._polyvore { background: transparent url(_cdn/images/attrib/polyvore.png) 182px 3px no-repeat; }",
